...
class ResearchOrchestrator:
    ...
    def __init__(self, project_dir: Path):
        ...
        self.manifest_status = verify_artifact_manifest(self.project_dir, self.artifact_dir)
        if self.manifest_status.get("status") == "changed":
            raise ValueError("artifact manifest mismatch; inspect changed files before resuming")
    ...
    def run_round(self, project: Project) -> Dict[str, object]:
        ...
        for index, target in enumerate(stages[start:], start=start):
            ...
            for agent in (item for item in agents if self._stage_agent(item, target, project)):
                ...
                write_artifact_manifest(self.project_dir, self.artifact_dir, project.project_id)
        self.save_project(project)
        return {"project": project.to_dict(), "artifacts": [item.to_dict() for item in self.artifacts]}
    ...
    def _stage_agent(agent: Agent, stage: Stage, project: Optional[Project] = None) -> bool:
        mapping = {
            ...
            Stage.EVIDENCE_REVIEW: {"evidence-synthesizer", "evaluation-subagent"},
            Stage.DRAFT: {"visual-planner", "visual-composer", "draft-writer", "composition-agent"},
        }
        ...
        if stage == Stage.EXPERIMENT and agent.name.startswith("gpu-subagent["):
            return True
