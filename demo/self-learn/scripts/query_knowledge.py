...
def _read_entries() -> Iterable[tuple[dict, Path, str]]:
...
def search(query: str, limit: int = 6, max_chars: int = 1800) -> list[dict]:
    wanted = _tokens(query)
    ranked = []
    for entry, path, text in _read_entries():
...
def main() -> int:
    ...
    results = search(args.query, args.limit, args.max_chars)
    if args.prompt:
